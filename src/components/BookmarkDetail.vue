...
<template>
  <div :style="{ padding: '16px', overflowY: 'auto', height: '100%' }">
    ...
    <!-- Tags -->
    <div v-if="bookmark.tags.length > 0" :style="{ display: 'flex', gap: '4px', flexWrap: 'wrap', marginBottom: '12px' }">
      <span
    ...
    >
      <div :style="{ fontSize: '11px', color: T.fgMuted, marginBottom: '4px', fontWeight: 600 }">Notes</div>
    ...
    <div :style="{ display: 'flex', gap: '8px', marginBottom: '16px' }">
      ...
      <button
        :style="{
          ...
          borderRadius: '4px',
          cursor: 'pointer',
          background: bookmark.fav ? T.yellow : T.bgHL,
          color: bookmark.fav ? T.bgDark : T.fgDim,
          border: 'none',
          fontWeight: 600,
        }"
        @click="emit('toggleFav', bookmark.id, !bookmark.fav)"
      >
        {{ bookmark.fav ? '★ Favorited' : '☆ Favorite' }}
      </button>
    </div>

    <!-- Timestamps -->
    <div :style="{ fontSize: '11px', color: T.fgMuted }">
      <div>Added: {{ bookmark.created }}</div>
      <div>Updated: {{ bookmark.updated }}</div>
    </div>
  </div>
</template>
